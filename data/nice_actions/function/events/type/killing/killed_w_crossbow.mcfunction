advancement revoke @s only nice_actions:events/killing/crossbow
scoreboard players add @s nice_actions.events.counter 0

execute \
if data storage eden:events nice_actions{killing:"crossbow"} \
...
run function nice_actions:events/payout with storage eden:settings nice_actions.events.killing