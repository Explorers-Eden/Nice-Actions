advancement revoke @s only nice_actions:events/fishing/tropical_fish
scoreboard players add @s nice_actions.events.counter 0

execute \
if data storage eden:events nice_actions{fishing:"tropical_fish"} \
...
run function nice_actions:events/payout with storage eden:settings nice_actions.events.fishing